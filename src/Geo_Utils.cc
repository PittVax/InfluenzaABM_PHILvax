...
const phil::geo MEAN_US_LON = -97.0; // near Wichita, KS
const phil::geo MEAN_US_LAT = 38.0; // near Wichita, KS
...
void Geo_Utils::set_km_per_degree(phil::geo lat) {
    lat *= Geo_Utils::DEG_TO_RAD;
    km_per_deg_longitude = cos(lat) * KM_PER_DEG_LAT;
    km_per_deg_latitude = KM_PER_DEG_LAT;
}

double Geo_Utils::haversine_distance(phil::geo lon1, phil::geo lat1, phil::geo lon2, phil::geo lat2) {
    // convert to radians
    lat1 *= DEG_TO_RAD;
    lon1 *= DEG_TO_RAD;
    lat2 *= DEG_TO_RAD;
...
double Geo_Utils::spherical_cosine_distance(phil::geo lon1, phil::geo lat1, phil::geo lon2, phil::geo lat2) {
    // convert to radians
    lat1 *= DEG_TO_RAD;
    lon1 *= DEG_TO_RAD;
    lat2 *= DEG_TO_RAD;
...
double Geo_Utils::spherical_projection_distance(phil::geo lon1, phil::geo lat1, phil::geo lon2, phil::geo lat2) {
    // convert to radians
    lat1 *= DEG_TO_RAD;
    lon1 *= DEG_TO_RAD;
    lat2 *= DEG_TO_RAD;
