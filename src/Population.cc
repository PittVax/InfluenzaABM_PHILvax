...
Population::Population() {
    ...
    vacc_manager = NULL;
    mutation_prob = NULL;
    // this is a hack which assumes that the initial population will never exceed 500M
    // this allows identification of any agents that are born in the simulation since
    // they will have an id >= 500M
    next_id = 500000000; 
...
void Population::setup() {
    ...
    if (Global::Verbose > 0) {
        ...
        for (int p = 0; p < pop_size; p++) {
            Disease * d = &disease[ 0 ];
            if (blq.get_item_reference_by_index(p).get_health()->is_immune(d)) {
                count++;
            }
        }
...
void Population::initialize_disease_state_counts_by_block() {
    ...
    for (int p=0; p<this->pop_size; p++) {
        ...
        // Must handle residual immunity here because it is not initialized yet
        if (pop_i.get_health()->is_immune(0)) {
            Global::Block_Epi_Day_Tracker->increment_index_key_pair(census_block,"M",int(1));
        } else {
            Global::Block_Epi_Day_Tracker->increment_index_key_pair(census_block,"S",int(1));
        }
    }
    Global::Block_Tracker_Initialized = true;
}
