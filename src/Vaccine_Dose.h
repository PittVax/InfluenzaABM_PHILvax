...
#ifndef _PHIL_VACCINEDOSE_H
#define _PHIL_VACCINEDOSE_H
...
class Vaccine_Dose {
  ...
  public:
    ...
    //Parameter Access
    Age_Map* get_efficacy_map()       const {
        return efficacy;
    }
    Age_Map* get_efficacy_delay_map() const {
        return efficacy_delay;
    }

    double  get_efficacy(int age)         const {
        return efficacy->find_value(age);
    }
    double  get_efficacy_delay(int age)   const {
        return efficacy_delay->find_value(age);
    }
    int     get_days_between_doses()  const {
        return days_between_doses;
    }
